...
class DaftarLocation extends StatefulWidget {
  const DaftarLocation({Key? key});

  @override
  State<DaftarLocation> createState() => _DaftarLocationState();
}

class _DaftarLocationState extends State<DaftarLocation> {
  ...
  String _selectedPlaceType = 'Hospital'; // Default selection
        ...
        children: [
          ...
          Padding(
            ...
            child: DropdownButton<String>(
              ...
              items: ['Hospital', 'Clinic', 'Health Center']
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  subtitle: Column(
                    ...
                    children: [
                      Text(selectedList[index]['address'] ?? ''),
                      Text('${selectedList[index]['distance']} KM away'),
                    ],
                  ),
                  trailing: ElevatedButton(
                    onPressed: () {
                      _onFollowButtonPressed(selectedList[index]);
                    },
  ...
  void updateMarkers() async {
    ...
    //Hospital
    if (_selectedPlaceType == 'Hospital') {
      ...
      //Clinic
    } else if (_selectedPlaceType == 'Clinic') {
      final bitmapDescriptor = await BitmapDescriptor.fromAssetImage(
      ...
      //HealthCenter
    } else if (_selectedPlaceType == 'Health Center') {
      final bitmapDescriptor = await BitmapDescriptor.fromAssetImage(
          const ImageConfiguration(devicePixelRatio: 2.5),
          'img/icon/kuning.png');
      _markers.addAll([
  ...
  void updateLists() {
    ...
    for (Marker marker in _markers) {
      ...
      if (marker.markerId.value.contains('Rumah Sakit')) {
        _hospitalList.add(place);
      } else if (marker.markerId.value.contains('Clinic') ||
    ...
    _hospitalList.sort((a, b) => a['distance'].compareTo(b['distance']));
